...
export const metadata = {
    name: "web_browser",
    description: "Công cụ điều khiển trình duyệt web ngầm định theo cơ chế Tự trị (Agentic). Gồm các hành động: navigate, click, type, extract, close. Giúp LIVA đọc HTML, trích xuất dữ liệu, và thao tác trên mọi website giống như con người.",
    parameters: {
        type: "object",
        properties: {
            action: {
            ...
            url: {
                type: "string",
                description: "Dùng cho hành động 'navigate': Đường dẫn trang web cần mở."
            },
            selector: {
                type: "string",
                description: "Dùng cho hành động 'click', 'type', hoặc 'extract': query selectors của phần tử (VD: 'button.submit', '#search-input', '.article')."
            },
            text: {
                type: "string",
                description: "Dùng cho hành động 'type': Nội dung chữ/text cần gõ vào ô input (VD: Từ khóa cần tìm)."
            }
        },
        required: ["action"]
    }
...
export const execute = async (args: { action: string, url?: string, selector?: string, text?: string }): Promise<string> => {
    try {
        ...
        switch (args.action) {
            case 'navigate': {
                if (!args.url) return 'Lỗi: Thiếu tham số bắt buộc `url` cho hành động navigate.';
            ...
            case 'click': {
                if (!args.selector) return 'Lỗi: Thiếu tham số `selector` cho hành động click.';
                ...
                await pageInstance.waitForNavigation({ waitUntil: 'networkidle2', timeout: 3000 }).catch(() => {});
            ...
            case 'type': {
                if (!args.selector || !args.text) return 'Lỗi: Thiếu tham số `selector` hoặc `text` cho hành động type.';
                await pageInstance.waitForSelector(args.selector, { timeout: 10000 });
            ...
            case 'extract': {
                ...
                if (args.selector) {
                    ...
                    extractedText = await pageInstance.evaluate(() => document.body.innerText.substring(0, 5000));
                }
