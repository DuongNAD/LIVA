...
- **Telegram remote control:** Real and working — starts with the standalone `liva-native-core` when `TELEGRAM_BOT_TOKEN` + `TELEGRAM_ALLOWED_IDS` are set. Not available in the desktop build. The allow-list is **fail-closed**: an empty `TELEGRAM_ALLOWED_IDS` rejects *everyone*, including you.
  ⚠️ **Real gap, stated plainly:** the `/cat` and `/ls` commands read paths straight off the message with no sandboxing — unlike the MCP server, which pins every path under `LIVA_VAULT_PATH`. Anyone on the allow-list can read any file the process can, over the Internet. Treat the allow-list as the only barrier and keep it to your own account.
