...
6. **Post-Mortem Documentation**:
   - Record incident findings and prevention runbooks into `vault/Knowledge/DevOps - <Incident_Title>.md` using `write_markdown`.
   - Adhere strictly to the Obsidian frontmatter standard (`title`, `tags: [knowledge/devops, ops/incident]`, `author: "user"`, `last_update`).

## Platform Constraints

- **Execution Mode**: Strict Two-Phase Confirmation for mutations; Auto-execution for read-only status and log inspections.
- **Tool Dependencies**: Requires `run_command` (shell execution harness), `gitnexus` MCP tools (`impact`, `detect_changes`), and `obsidian` MCP tools (`write_markdown`, `search_vault`).
...
# DevOps Incident: Docker Memory Contention Triage

## Summary
The `liva-native-core` test container exited with Code 137 (OOMKilled) during simultaneous integration and ONNX embedding probe runs.

## Root Cause
- Default Docker Compose memory limit was set to 512MB (`mem_limit: 512m`).
- Peak memory during dual ONNX model loading (`multilingual-e5-small` + `parakeet_tdt`) reached 780MB.

## Verification & Remediation Plan
1. **Container Diagnostic**:
   ```powershell
   docker inspect liva_test_runner --format '{{.State.OOMKilled}}'
   # Output: true
   ```
2. **Configuration Patch**:
   - Update `docker-compose.test.yml`:
     ...
     ```
3. **Validation**:
   - Re-run test suite in container: `cargo test --lib -- --nocapture` -> PASS.
```
