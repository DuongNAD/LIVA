...
- **Backlog thi hành: [`docs/03-danh-gia/05-nang-cap-toan-dien.md`](docs/03-danh-gia/05-nang-cap-toan-dien.md).** Read it before proposing work of your own. It carries a measured baseline (re-run it first — a number that dropped is a regression and outranks everything in the backlog), prioritized items **U1–U33** in six groups A–F, each with a *verifiable* acceptance condition, and a **§9** "do NOT do this" list that exists to stop sessions burning time on plausible-looking non-work. ⚠️ **Read the §2 table, not the numbers** — the U16–U20 range was claimed by U21+ first, so the item number no longer tells you which group an item belongs to.
...
- Doc conventions (front-matter schema, `[OK]`/`[MỘT PHẦN]`/`[THIẾU]` labels, the "no invented numbers" rule): [`docs/README.md`](docs/README.md). Both `docs-check.mjs` and `docs-citations.mjs` are CI gates — run them after touching anything under `docs/`, and **pass the same flag CI does** or you will not reproduce its result:
  ...
  ```
  The second one has its own budget flag and is a **separate** CI step — `npm run docs:check` does **not** run it, so a coordinate pointing at a file you just moved or deleted survives a green `docs:check`. Use `npm run docs:cite` locally.
...
- CI (`.github\workflows\test.yml`, windows-latest) — **25 steps, every step a gate**: `docs-check.mjs --strict-stale=docs/03-danh-gia` → `docs-citations.mjs` → `npm ci` → **`npm run devkit:lint`** → **`npm audit --audit-level=high`** → **`cargo fmt --all -- --check`** → cargo cache → **`cargo install cargo-deny --version 0.20.2 --locked`** → **`cargo deny check -W unmaintained -W unsound advisories licenses sources`** → LLVM → `vue-tsc --noEmit -p tsconfig.app.json` → `eslint --max-warnings 0` → `npm run test:coverage -w liva-ui` (vitest + istanbul coverage gate) → build web clients → validate knowledge vault → `cargo test` → build gateway binary → **`node scripts/e2e-gateway-ci.mjs`** (gateway thật qua socket, build debug) → **`cargo test -p liva-desktop`** → `cargo check -p liva-desktop` → `cargo check --all-targets --features experimental` → clippy. Six of those are new as of 2026-08-01 (`98efc55`, marked in bold above) and all 25 went green on `e6391eb`. A second workflow `.github\workflows\release.yml` (tag `v*` · manual · weekly) builds `cargo build --release` + `npx tauri build` and re-runs the e2e against the **release** binary — that is the only path where `vision:ask` actually works.
  - **`cargo fmt --all -- --check` IS a hard gate now** (added 2026-08-01). This file previously said "No fmt gate" — that was true until `98efc55` and is the kind of claim to re-read the workflow for rather than trust.
  - **The Rust advisory gate is `cargo deny check -W unmaintained -W unsound advisories licenses sources` since 2026-08-08 (`f35961cf`)** — it replaced `cargo audit`, adding license + source compliance; the two `-W` flags preserve the old behavior, so the measurements below still read across the swap. **Only vulnerabilities fail it.** Measured 2026-08-01 (with `cargo audit`): exit 0 with **22 `unmaintained`/`unsound` warnings** from the Tauri/GTK tree (atk, atk-sys, bincode…). That count drifts as RustSec publishes advisories, so a change in it is not by itself a regression in this repo — check whether the new entry is reachable from LIVA's own code before acting. Confirmed again on 2026-08-04, this time with teeth: `npm audit --audit-level=high` went from 0 to **5 findings (3 high)** in two days with `package-lock.json` untouched, which turned CI red at step 8 of 25 and left the other 17 steps unrun. Proof it was drift and not a regression: stash the whole tree and run it on clean HEAD — also red. Fixed by `npm audit fix` alone (5 semver-compatible bumps, lockfile only, `30349c5`).
  - **Always run the advisory gate from the workspace root.** Until 2026-08-04 a stale pre-workspace `liva-native-core\Cargo.lock` was still tracked, and running `cargo audit` from inside `liva-native-core\` resolved *that* file instead: **319 crates and a confident "1 vulnerability"** (crossbeam-epoch RUSTSEC-2026-0204), versus **857 crates and 0 vulnerabilities** from the root, which is what CI measures. Same class of leftover as `liva-native-core\target\`. The file is now deleted and gitignored, so that command fails loudly instead (`error: not found: Couldn't load Cargo.lock`) — a legible error rather than a confidently wrong answer, but still a surprise if you are expecting a report.
  - **`docs-check.mjs` also validates `#anchor` links** (added 2026-08-01, §3a). It caught 4 broken anchors on its first run. The slugger has two traps documented in place — `trim()` must run *before* the character-strip, and each space maps to exactly one `-`, no collapsing.
  - **clippy is a HARD gate** (`-- -D warnings`, 0 warnings since 2026-07-22 — journey 80 → 35 via `--fix`, then 35 → 0 by hand with provably-equivalent rewrites; DSP loop rewrites were additionally verified by a seed-42 VieNeu WAV hash, byte-identical. Remaining `#[allow]`s are deliberate with in-place justifications).
  - **Two measurement traps, both of which produced a false all-clear before 2026-07-22 — treat any always-green check as suspect.** (1) Clippy: measure with `--message-format=short` and grep `": warning:"`; the short format prefixes paths with `liva-native-core\src\…`, so a naive `grep '^src/'` reports zero. (2) Typecheck: `liva-ui/tsconfig.json` is a solution-style config (`"files": []` + references), so plain `tsc --noEmit` checks **zero files** — and plain `tsc` can't read `.vue` SFCs either. Always use `vue-tsc --noEmit -p tsconfig.app.json`.
