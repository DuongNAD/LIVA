...
async function main() {
    ...
    console.log("=========================================");
    console.log("Môi trường này cho phép chạy và kiểm thử trực tiếp các chức năng");
    ...
    while (true) {
        ...
        if (argsStr.trim() !== '') {
            try {
                ...
                console.log(`⚠️ Lỗi nhập liệu JSON: ${e.message}. Hãy gõ JSON chuẩn (nháy kép ở key và biến string).`);
                continue;
            }
        } else {
            // Force location to be empty string so it triggers Auto-IP
            console.log(">> Tham số bị bỏ trống. Ép hệ thống dùng giá trị mặc định / tự động lấy Vị trí.");
            args = {};
        }
