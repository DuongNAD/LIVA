...
logger.info(`[CI Coverage] Found test directories: ${dirs.join(', ')}`);
...
for (const dir of dirs) {
  ...
  try {
    ...
    logger.error(`[CI Coverage] Error running tests in tests/${dir}`);
    hasError = true;
  }
}
...
try {
  ...
  // Fix paths in lcov.info for SonarQube (prepend liva-gateway/ to SF:src/ paths)
  const lcovPath = path.join(COVERAGE_DIR, 'lcov.info');
  if (fs.existsSync(lcovPath)) {
    let lcovContent = fs.readFileSync(lcovPath, 'utf8');
    // Handle both Windows (\) and Unix (/) paths
    lcovContent = lcovContent.replace(/^SF:src[\\/]/gm, 'SF:liva-gateway/src/');
    ...
    logger.info('[CI Coverage] Fixed paths in lcov.info for SonarQube compatibility.');
  }
