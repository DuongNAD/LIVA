...
export const execute = async (args: { command: string }): Promise<string> => {
    try {
        console.log(`[Skill: execute_command] Đang phân tích lệnh (Analyzing): ${args.command}`);
        ...
        const DANGEROUS_PATTERNS = [
            /rmdir/i, /rd\s+\/s/i, /del\s+\/f/i, /Remove-Item/i, // Lệnh xóa/phá hủy tệp lớn
            /format\s+[a-z]:/i, /diskpart/i, /mkfs/i,          // Lệnh can thiệp phân vùng ổ đĩa
            /shutdown/i, /Stop-Computer/i, /Restart-Computer/i,// Lệnh điều khiển nguồn
            /Uninstall/i, /msiexec\s+\/[xX]/i, /reg\s+delete/i,// Lệnh gỡ phần mềm/phá Registry
            /netsh/i, /ipconfig\s+\/(release|renew)/i, /route\s+delete/i, /Disable-NetAdapter/i, // Lệnh Phá Internet/Mạng
            /net\s+user/i, /Set-LocalUser/i, /syskey/i,        // Lệnh Quản trị Account
            /taskkill\s+\/f/i, /Stop-Process/i,                // Lệnh giết tiến trình cưỡng bức
            /Set-ExecutionPolicy/i, /Clear-EventLog/i          // Lệnh can thiệp lõi bảo mật
        ];

        for (const pattern of DANGEROUS_PATTERNS) {
            if (pattern.test(args.command)) {
                console.warn(`[SECURITY ALERT] Liva phát hiện và chặn lệnh sinh tử: ${args.command}`);
        ...
        console.log(`[Skill: execute_command] Lệnh an toàn. Đang chạy (Executing)...`);
