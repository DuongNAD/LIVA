...
| **P2** ◐ | KDF + fail-closed decrypt (C3) — *22/07: KDF thật (HKDF+salt, định dạng v2) + migration re-encrypt v1→v2 KHÔNG mất dữ liệu + primitive `try_decrypt` ĐÃ XONG. Còn: đường đọc `get_fact` vẫn fail-open, bỏ default key — cần quyết định* | — | Mã hoá hiện gần như trang trí | 1 ngày | ★★★ |
