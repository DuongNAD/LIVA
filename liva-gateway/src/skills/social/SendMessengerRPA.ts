...
export const metadata = {
  ...
  search_keywords: ["send_messenger_rpa","send messenger rpa","gửi","nhắn tin","send message","facebook","messenger","chat","message friend","tin nhắn","nhắn mess","gửi tin"],
    description: "[ASK_FIRST] Browser Automation (RPA) to send a message via Facebook Messenger Web. Use when user asks to send a message on Messenger.",
    kit: "SOCIAL_KIT",
    requires_hitl: true,
    parameters: {
        type: "object",
        properties: {
            targetName: { type: "string", description: "Recipient name. MUST strip honorifics/pronouns (e.g., 'mr. Vu' -> 'Vu', 'friend Hung' -> 'Hung'). DO NOT split or remove parts of compound/full names (e.g., 'Đức Khánh' MUST remain 'Đức Khánh')." },
            message: { type: "string", description: "[LOCALIZED] UNDERSTAND INTENT, DO NOT COPY VERBATIM. ROLEPLAY as user and REWRITE naturally. CRITICAL: DO NOT add personality/submissiveness (NO 'Dạ', 'ạ', 'em'). Write as if the USER is chatting with peers/family. Use casual language matching user's language." }
        },
