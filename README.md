...
- 👁️ **Native screen vision.** A pure-Rust screen capture (Windows Graphics Capture via `xcap`) and region-diff engine (`vision:capture`, `vision:get_changed_regions`) watches selected screen regions with minimal overhead. `vision:ask` sends a frame to the multimodal Qwen3-VL model for visual question answering — entirely local.
  **Two things to know, both measured rather than guessed (2026-07-26).**
  ...
  **The catch is distribution, not speed.** The CUDA build takes **20 min** to compile and produces a **202 MB** binary (vs 43 MB), because `llama-cpp-sys-2` never pins `CMAKE_CUDA_ARCHITECTURES`, so every default GPU generation gets compiled in. It also links `cudart`, so it is useless on a machine without an NVIDIA GPU. Which build to actually ship is an open decision, not a solved problem.
