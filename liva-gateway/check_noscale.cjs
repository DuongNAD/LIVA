// Quick re-test with NO *32768 scaling
...
function computeMelFilterbank() {
    const n = 257; const mlMin = hzToMel(0), mlMax = hzToMel(8000);
    const mp = new Float32Array(130); for (let i = 0; i < 130; i++) mp[i] = mlMin + i * (mlMax - mlMin) / 129;
    const bi = new Float32Array(130); for (let i = 0; i < 130; i++) bi[i] = melToHz(mp[i]) * 512 / 16000;
    const f = []; for (let m = 0; m < 128; m++) {
        const fl = new Float32Array(n), l = bi[m], c = bi[m+1], r = bi[m+2];
        for (let k = 0; k < n; k++) { if (k >= l && k <= c && c > l) fl[k] = (k-l)/(c-l); else if (k > c && k <= r && r > c) fl[k] = (r-k)/(r-c); }
        f.push(fl);
    } return f;
}
function fft(frame) {
...
async function main() {
    ...
    for (let i = 0; i < N; i++) { const t = i/sr; raw[i] = 0.3*Math.sin(2*Math.PI*200*t) + 0.2*Math.sin(2*Math.PI*400*t) + 0.15*Math.sin(2*Math.PI*600*t) + 0.1*Math.sin(2*Math.PI*1000*t) + 0.05*(Math.random()-0.5); }

    // NO *32768 — raw float [-1, 1]
    ...
    for (let f = 0; f < 65; f++) { const o = f * 160;
        ...
        const ps = fft(wf);
        for (let m = 0; m < 128; m++) { let e = 0; for (let k = 0; k < 257; k++) e += mf[m][k]*ps[k]; feat[f*128+m] = Math.log(e + 5.96046448e-08); }
    }

    let mn = Infinity, mx = -Infinity, sm = 0;
    for (let i = 0; i < feat.length; i++) { mn = Math.min(mn, feat[i]); mx = Math.max(mx, feat[i]); sm += feat[i]; }
    console.log(`Mel (no scale): [${mn.toFixed(2)}, ${mx.toFixed(2)}] mean=${(sm/feat.length).toFixed(2)}`);
    ...
    for (let t = 0; t < outLen; t++) {
        ...
        if (t === 0) { console.log(`Frame 0 top-5: ${idx.slice(0,5).map(x=>`[${x.i}]=${x.v.toFixed(3)}`).join(' ')}`); console.log(`  blank=${lg[config.blankId]?.toFixed(3)}`); }
        if (idx[0].i !== config.blankId) { tokens++; console.log(`  t=${t}: tok=${idx[0].i}`); }
    }
