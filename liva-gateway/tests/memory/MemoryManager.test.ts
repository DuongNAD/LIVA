...
describe("MemoryManager", () => {
    ...
    describe("v29.1 Architectural Fixes", () => {
        it("should successfully mark the last dialogue turn as reflected", async () => {
            await mm.addMessage("user", "Hello");
            await mm.addMessage("assistant", "Hi there!");
            await mm.addMessage("user", "Follow up question");
            
            mm.markLastTurnReflected();
            
            const history = await mm.getShortTermHistory();
            expect(history[0].reflected).toBe(true);      // First user turn marked (part of completed turn)
            expect(history[1].reflected).toBe(true);      // Last assistant turn marked (part of completed turn)
            expect(history[2].reflected).toBeUndefined(); // Follow-up user turn not marked yet
        });

        it("should not re-queue already reflected messages when trimming cache", async () => {
            await mm.initialize();
            const mockDaemon = {
                queueTurn: vi.fn(),
            ...
            (mm as any).reflectionDaemon = mockDaemon;

            // Add 60 messages, all marked as reflected
            for (let i = 0; i < 30; i++) {
                await mm.addMessage("user", `Query ${i}`);
                const history = await mm.getShortTermHistory();
                history[history.length - 1].reflected = true;

                await mm.addMessage("assistant", `Response ${i}`);
                const history2 = await mm.getShortTermHistory();
                history2[history2.length - 1].reflected = true;
            }

            expect(mockDaemon.queueTurn).not.toHaveBeenCalled();
        });

        it("should truncate single message when it exceeds MAX_MSG_TOKENS", async () => {
            const longText = "word ".repeat(4000); // 4000 words * 1.5 = 6000 tokens
            await mm.addMessage("user", longText);
            
            const ctx = await mm.getHybridContext("query", 1);
            expect(ctx.length).toBe(1);
            expect(ctx[0].content).toContain("[...truncated]");
            expect(ctx[0].content.length).toBeLessThan(longText.length);
        });
    });

    describe("Edge Cases (TC-16, TC-17, TC-18)", () => {
        it("should detect and clean up dangling temporary files (.tmp) on initialize (TC-16)", async () => {
            const agentDir = path.join(process.cwd(), "data", "agents", "test-agent");
            
            // Khởi tạo các tệp tạm bị treo trong memfs trước khi initialize
            const fsp = await import("node:fs/promises");
            await fsp.mkdir(agentDir, { recursive: true });
            await fsp.writeFile(path.join(agentDir, "SESSION-STATE.md.tmp"), "dangling state content");
            await fsp.writeFile(path.join(agentDir, "other_file.tmp"), "dangling other content");
            
            // Xác nhận các tệp này tồn tại trước khi khởi tạo
            await expect(fsp.access(path.join(agentDir, "SESSION-STATE.md.tmp"))).resolves.not.toThrow();
            await expect(fsp.access(path.join(agentDir, "other_file.tmp"))).resolves.not.toThrow();
            
            // Kích hoạt initialize
            await mm.initialize();
            
            // Đảm bảo các tệp tạm mồ côi đã bị dọn dẹp (xóa) sạch sẽ
            await expect(fsp.access(path.join(agentDir, "SESSION-STATE.md.tmp"))).rejects.toThrow();
            await expect(fsp.access(path.join(agentDir, "other_file.tmp"))).rejects.toThrow();
            
            // Đảm bảo tệp Session State chính vẫn được sinh ra bình thường
            await expect(fsp.access(path.join(agentDir, "SESSION-STATE.md"))).resolves.not.toThrow();
        });

        it("should operate stably without memory corruption or uncaught exceptions under high message spam (TC-17)", async () => {
            await mm.initialize();
            ...
            for (let i = 0; i < spamCount; i++) {
                ...
                promises.push(mm.addMessage(role, `Spam message ${i}`));
                
                // Đồng thời gọi getHybridContext liên tục
                if (i % 3 === 0) {
                    promises.push(mm.getHybridContext(`Spam query ${i}`));
                }
            }
            
            // Đảm bảo không ném ra bất kỳ lỗi bất đồng bộ nào
            await expect(Promise.all(promises)).resolves.not.toThrow();
            
            const history = await mm.getShortTermHistory();
            expect(history.length).toBeLessThanOrEqual(50);
            
            // Đảm bảo cấu trúc cache được giữ toàn vẹn, không chứa phần tử undef
            history.forEach(msg => {
        ...
        it("should budget accurately with custom/mocked token counts (TC-18)", async () => {
            await mm.initialize();
            
            // Mock estimateTokens để trả về chính xác số lượng token định nghĩa trước
            const spy = vi.spyOn(MemoryManager, "estimateTokens").mockImplementation((text: string) => {
                if (text.includes("TruncateMe")) return 4000; // Vượt ngưỡng MAX_MSG_TOKENS (3000)
                if (text.includes("RAG")) return 1000;
                return 100;
            });

            await mm.addMessage("user", "Normal message");
            await mm.addMessage("user", "TruncateMe long message...");
            
            // Lấy ngữ cảnh hybrid
            const ctx = await mm.getHybridContext("query", 6);
            
            // Kiểm tra tin nhắn TruncateMe đã bị cắt gọn
            const truncatedMsg = ctx.find(m => m.content.includes("TruncateMe"));
            expect(truncatedMsg).toBeDefined();
            expect(truncatedMsg!.content).toContain("[...truncated]");
