...
export const metadata = {
    name: "send_zalo_bot",
    description: "Gửi tin nhắn thông báo, tóm tắt qua Zalo thông qua Zalo Official Account (OA) API. Siêu nhanh và chạy ngầm hoàn toàn. Phù hợp để gửi tin nhắn cho 'Bot ClawAI Dương'.",
    parameters: {
        type: "object",
        properties: {
            message: { 
...
export const execute = async (args: { message?: string, text?: string, content?: string }): Promise<string> => {
    try {
        console.log(`[Skill: send_zalo_bot] Đang chuẩn bị gửi tin nhắn qua Zalo Bot API...`);
        ...
        if (isBotCreatorToken) {
            ...
            if (!userId || userId.includes("NHẬP_USER_ID")) { 
                console.log(`[Skill: send_zalo_bot] Dùng Bot mới nhưng chưa có User ID. Đang quét API dò tìm ID tự động...`);
                try {
                    ...
                    if (updateRes.data && updateRes.data.ok && updateRes.data.result && updateRes.data.result.message) {
                        ...
                        console.log(`[Skill: send_zalo_bot] Magic! Đã tự động bắt được User ID mới: ${userId}`);
                    } else {
            ...
            if (response.data && response.data.ok) {
                console.log(`[Skill: send_zalo_bot] Gửi tin nhắn thành công qua hệ mới Bot Creator!`);
            ...
            if (response.data && response.data.error === 0) {
                console.log(`[Skill: send_zalo_bot] Gửi tin nhắn thành công qua Zalo OA!`);
