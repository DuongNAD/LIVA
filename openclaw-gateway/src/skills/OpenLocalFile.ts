...
export const metadata = {
    ...
    description: "Mở một tệp tin (hoặc thư mục) cục bộ bằng phần mềm mặc định của hệ điều hành Windows (Ví dụ: Mở file Word, trình duyệt, hình ảnh, hoặc File Explorer).",
    parameters: {
        type: "object",
        properties: {
            targetPath: {
                type: "string",
                description: "Đường dẫn tuyệt đối hoặc tương đối tới tệp tin/thư mục cần mở. Ví dụ: 'D:/LIVA/package.json' hoặc 'D:/Downloads'"
            }
        },
        required: ["targetPath"]
    }
...
export const execute = async (args: { targetPath: string }): Promise<string> => {
    try {
        const absolutePath = path.resolve(process.cwd(), args.targetPath);
        console.log(`[Skill: open_local_file] Đang kích hoạt mở (Opening): ${absolutePath}`);
