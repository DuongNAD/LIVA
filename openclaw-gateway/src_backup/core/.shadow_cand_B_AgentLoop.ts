...
export class AgentLoop {
    ...
    public dispatch(task: MessageTask, token: AuthorityToken<AgentPhase>): void {
        if (!this.#authority.verify(token, this.#currentPhase)) { throw new Error("Unauthorized Task Dispatch! Invalid Authority Token."); } const queue = this.#lanes.get(task.lane); if (queue) { queue.push(task); this.dispatchEvent('taskDispatched', task); }
    }
