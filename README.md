...
- 👁️ **Native screen vision.** A pure-Rust screen capture (Windows Graphics Capture via `xcap`) and region-diff engine (`vision:capture`, `vision:get_changed_regions`) watches selected screen regions with minimal overhead. `vision:ask` sends a frame to the multimodal Qwen3-VL model for visual question answering — entirely local.
  **Two limits, measured rather than guessed (2026-07-26).** (1) On Windows `vision:ask` needs a **release build**: CMake compiles llama.cpp against the debug CRT in a debug profile while Rust always links the release one, and the resulting file-descriptor mismatch aborts inside the mmproj loader. A guard turns that abort into a clean error, so a debug build tells you rather than crashing. (2) On release it genuinely works — it read a browser page, a post and a chat participant's name off a real screen — but it costs **~80 s per question**, flat across calls (80.2 / 81.3 / 79.0 s), so it is per-image work and not first-call warmup. llama.cpp attributes it to ~56 s encoding the image and ~29 s decoding 2040 image tokens, all on **CPU**: this build is not compiled with `--features cuda`, so a GPU sits unused. Treat screen vision as *working but not yet usable in conversation*; a CUDA build is the obvious next lever and has not been measured.
