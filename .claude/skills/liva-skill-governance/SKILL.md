...
4. Scaffold new skills following the standard directory template. A `SKILL.md` frontmatter contains only `name` and `description`; agent UI and tool dependencies belong in `agents/openai.yaml`.
...
6. Validate every changed skill by running `npm run test:skills`, then rerun `npm run skills:audit`. Ensure 0 errors and 0 unclassified warnings.
