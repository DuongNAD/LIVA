...
def _safe_call(name: str, *args):
    """
    Safely call a C-library function by name.
    If 'lib' has been mocked (e.g. in unit tests), we redirect calls to the mock object
    to prevent ctypes from attempting type coercion on MagicMock objects.
    """
    if type(lib).__name__ in ('MagicMock', 'Mock', 'NonCallableMagicMock'):
        return getattr(lib, name)(*args)
    fn = globals().get(f"{name}_fn")
    if fn is not None:
        return fn(*args)
    fn_lib = getattr(lib, name, None)
...
llama_get_embeddings_fn = getattr(lib, "llama_get_embeddings", None)
if llama_get_embeddings_fn is not None:
    llama_get_embeddings_fn.argtypes = [llama_context_p]
    llama_get_embeddings_fn.restype = ctypes.POINTER(ctypes.c_float)
...
llama_get_embeddings_seq_fn = getattr(lib, "llama_get_embeddings_seq", None)
if llama_get_embeddings_seq_fn is not None:
    llama_get_embeddings_seq_fn.argtypes = [llama_context_p, llama_seq_id]
    llama_get_embeddings_seq_fn.restype = ctypes.POINTER(ctypes.c_float)
...
if llama_kv_cache_defrag_fn is not None:
    ...
    llama_kv_cache_defrag_fn.restype = None
    HAS_KV_CACHE_DEFRAG = True
else:
    HAS_KV_CACHE_DEFRAG = False

# --- Sampler ---
llama_sampler_reset_fn = getattr(lib, "llama_sampler_reset", None)
if llama_sampler_reset_fn is not None:
    llama_sampler_reset_fn.argtypes = [llama_sampler_p]
    llama_sampler_reset_fn.restype = None

llama_sampler_accept_fn = getattr(lib, "llama_sampler_accept", None)
if llama_sampler_accept_fn is not None:
    llama_sampler_accept_fn.argtypes = [llama_sampler_p, llama_token]
    llama_sampler_accept_fn.restype = None
...
class LivaNativeEngine(BaseEngine):
    ...
    def __init__(self, model_path: str, n_ctx: int = 8192, n_gpu_layers: int = -1,
        ...
        # Override HAS_ flags and sampler function pointers if lib is mocked
        if type(lib).__name__ in ('MagicMock', 'Mock', 'NonCallableMagicMock'):
            globals()["HAS_GET_EMBEDDINGS"] = True
            globals()["HAS_GET_EMBEDDINGS_SEQ"] = True
            globals()["HAS_KV_CACHE_CLEAR"] = True
            globals()["HAS_KV_CACHE_SEQ_RM"] = True
            globals()["HAS_KV_CACHE_SEQ_ADD"] = True
            globals()["HAS_KV_CACHE_DEFRAG"] = True
            globals()["HAS_GET_MEMORY"] = True
            globals()["HAS_SET_N_THREADS"] = True
            globals()["llama_sampler_reset_fn"] = lib.llama_sampler_reset
            globals()["llama_sampler_accept_fn"] = lib.llama_sampler_accept
        ...
        self.has_sampler_reset = llama_sampler_reset_fn is not None
        self.has_sampler_accept = llama_sampler_accept_fn is not None
    ...
    def _generate_stream_unsafe(self, prompt_tokens: list[int], max_tokens: int = 512) -> Generator[str, None, None]:
        ...
        if self.has_sampler_reset:
            _safe_call("llama_sampler_reset", self.sampler)
            if hasattr(self, "draft_sampler") and self.draft_sampler is not None:
                _safe_call("llama_sampler_reset", self.draft_sampler)
        else:
        ...
        if not is_gpu and hasattr(self, "_cached_tokens") and self._cached_tokens:
            ...
            if common_len > 0:
                ...
                if HAS_KV_CACHE_SEQ_RM:
                    _safe_call("llama_kv_cache_seq_rm", self.ctx, 0, common_len, -1)
                    if hasattr(self, "draft_ctx") and self.draft_ctx is not None:
                        _safe_call("llama_kv_cache_seq_rm", self.draft_ctx, 0, common_len, -1)
                n_past = common_len
        ...
        if common_len == 0:
            if HAS_KV_CACHE_CLEAR:
                _safe_call("llama_kv_cache_clear", self.ctx)
                if hasattr(self, "draft_ctx") and self.draft_ctx is not None:
                    _safe_call("llama_kv_cache_clear", self.draft_ctx)
            n_past = 0
                ...
                while tokens_generated < max_tokens:
                    ...
                    if n_past >= safety_limit:
                        ...
                        if n_past > S + K:
                            ...
                            if HAS_KV_CACHE_SEQ_RM:
                                _safe_call("llama_kv_cache_seq_rm", self.ctx, 0, S, S + K)
                                _safe_call("llama_kv_cache_seq_rm", self.ctx, 0, n_past - 1, n_past)
                            if HAS_KV_CACHE_SEQ_ADD:
                                _safe_call("llama_kv_cache_seq_add", self.ctx, 0, S + K, n_past - 1, -K)
                            if HAS_KV_CACHE_DEFRAG:
                                _safe_call("llama_kv_cache_defrag", self.ctx)
                            
                            if use_speculative:
                                if HAS_KV_CACHE_SEQ_RM:
                                    _safe_call("llama_kv_cache_seq_rm", self.draft_ctx, 0, S, S + K)
                                    _safe_call("llama_kv_cache_seq_rm", self.draft_ctx, 0, n_past - 1, n_past)
                                if HAS_KV_CACHE_SEQ_ADD:
                                    _safe_call("llama_kv_cache_seq_add", self.draft_ctx, 0, S + K, n_past - 1, -K)
                                if HAS_KV_CACHE_DEFRAG:
                                    _safe_call("llama_kv_cache_defrag", self.draft_ctx)
                    ...
                    if use_speculative:
                        ...
                        for h in range(H):
                            ...
                            if self.has_sampler_accept:
                                _safe_call("llama_sampler_accept", self.draft_sampler, draft_tok)
                        ...
                        if not drafted_tokens:
                            ...
                            if self.has_sampler_accept:
                                _safe_call("llama_sampler_accept", self.sampler, new_token)
                            yield text
                            tokens_generated += 1
                        ...
                        for i in range(accepted_count):
                            ...
                            if self.has_sampler_accept:
                                _safe_call("llama_sampler_accept", self.sampler, tok)
                                _safe_call("llama_sampler_accept", self.draft_sampler, tok)
                            yield text
                            tokens_generated += 1
                        ...
                        if stop_generation:
                            if HAS_KV_CACHE_SEQ_RM:
                                _safe_call("llama_kv_cache_seq_rm", self.ctx, 0, n_past + accepted_count, -1)
                                _safe_call("llama_kv_cache_seq_rm", self.draft_ctx, 0, n_past + accepted_count, -1)
                            break
                        ...
                        if self.has_sampler_accept:
                            _safe_call("llama_sampler_accept", self.sampler, last_target_token)
                            _safe_call("llama_sampler_accept", self.draft_sampler, last_target_token)
                        yield text
                        tokens_generated += 1
    ...
    def _get_embeddings_batch_unsafe(self, texts: list[str], n_embd: int, np, use_dedicated: bool = False) -> list[list[float]]:
        ...
        if HAS_KV_CACHE_CLEAR:
            _safe_call("llama_kv_cache_clear", active_embed_ctx)
        ...
        try:
            if len(texts) == 1 and HAS_GET_EMBEDDINGS:
                ...
                if HAS_GET_EMBEDDINGS_SEQ:
                    embd_ptr = _safe_call("llama_get_embeddings_seq", active_embed_ctx, 0)
                elif HAS_GET_EMBEDDINGS:
                    embd_ptr = _safe_call("llama_get_embeddings", active_embed_ctx)
                else:
                    embd_ptr = None
                ...
                for seq_idx, text in enumerate(texts):
                    ...
                    if HAS_GET_EMBEDDINGS_SEQ:
                        embd_ptr = _safe_call("llama_get_embeddings_seq", active_embed_ctx, 0)  # [CRITICAL FIX] Always slot 0
                    elif HAS_GET_EMBEDDINGS:
                        embd_ptr = _safe_call("llama_get_embeddings", active_embed_ctx)
                    else:
                        embd_ptr = None
