...
export const metadata = {
  ...
  description: "[ASK_FIRST] Direct Computer Use (Windows/macOS). LIVA acts as a human: Move mouse, click, type on keyboard, and take screenshots. macOS mouse needs `cliclick` + Accessibility permission.",
  kit: "PERSONAL_KIT",
  parameters: {
    type: "object",
    properties: {
...
// macOS: run an AppleScript from a temp file (avoids shell-quoting issues for text).
async function runAppleScript(scriptContent: string): Promise<string> {
    const rpaDir = path.join(process.cwd(), "data", "rpa_scripts");
    await fs.mkdir(rpaDir, { recursive: true });
    const tempFile = path.join(rpaDir, `temp_rpa_${Date.now()}.scpt`);
    await fs.writeFile(tempFile, scriptContent, "utf-8");
    try {
        const { stdout } = await execAsync(`osascript "${tempFile}"`, { timeout: 10000 });
        return stdout.trim();
    } finally {
        await fs.unlink(tempFile).catch(() => {});
    }
...
export const execute = async (argsObj: any): Promise<string> => {
    try {
        ...
        // macOS: chuột dùng `cliclick` (brew install cliclick); gõ phím dùng osascript
        // System Events keystroke (zero-dep). Cần cấp quyền Accessibility cho Terminal.
        ...
        if (parsed.action === "take_screenshot") {
            ...
            if (isMac) {
                await execAsync(`screencapture -x "${filePath}"`, { timeout: 15000 });
            } else {
                const psScript = `
        ...
        if (parsed.action === "mouse_move") {
            ...
            if (isMac) {
                try {
                    await execAsync(`cliclick m:${Math.round(parsed.x)},${Math.round(parsed.y)}`, { timeout: 5000 });
                } catch {
                    throw new Error("Không chạy được cliclick. Cài bằng: brew install cliclick (và cấp quyền Accessibility cho Terminal).");
                }
            } else {
                const psScript = `
                    $code = @'
...
'@
        ...
        if (parsed.action === "mouse_click") {
            if (isMac) {
                // cliclick: c = left click, rc = right click, dc = double click; ":." = current cursor pos.
                const verb = parsed.button === "right" ? "rc" : parsed.button === "double" ? "dc" : "c";
                try {
                    await execAsync(`cliclick ${verb}:.`, { timeout: 5000 });
                } catch {
                    throw new Error("Không chạy được cliclick. Cài bằng: brew install cliclick (và cấp quyền Accessibility cho Terminal).");
                }
            } else {
...
'@
        ...
        if (parsed.action === "type_text") {
            ...
            if (isMac) {
                // osascript keystroke via temp .scpt — escape backslash/quote, flatten newlines.
                const escaped = parsed.text.replace(/\\/g, "\\\\").replace(/"/g, '\\"').replace(/\r?\n/g, " ");
