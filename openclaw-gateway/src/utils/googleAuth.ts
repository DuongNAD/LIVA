...
export async function getGoogleAuthClient() {
    if (!fs.existsSync(CREDENTIALS_PATH)) {
        throw new Error(`[Google Auth] Không tìm thấy file credentials.json tại ${CREDENTIALS_PATH}. Vui lòng tạo trên Google Cloud Console.`);
    }
    ...
    if (!fs.existsSync(TOKEN_PATH)) {
        throw new Error(`[Google Auth] Chưa được cấp quyền OAuth2 (Không tìm thấy token.json). Vui lòng chạy lệnh: npx tsx src/utils/auth_google_script.ts để đăng nhập lần đầu.`);
    }
