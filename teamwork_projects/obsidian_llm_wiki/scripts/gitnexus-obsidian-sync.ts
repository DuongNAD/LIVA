...
function getCleanSymbolName(symbol: string): string {
...
function main() {
  ...
  if (incomingCalls.length === 0) {
    ...
    incomingCallsContent = `| Symbol Name | File Path | Unique Identifier |\n| :--- | :--- | :--- |\n` +
      incomingCalls.map((c: any) => `| \`${c.name}\` | \`${c.filePath}\` | \`${c.uid}\` |`).join('\n');
  }
  ...
  if (outgoingCalls.length === 0) {
    ...
    outgoingCallsContent = `| Symbol Name | File Path | Unique Identifier |\n| :--- | :--- | :--- |\n` +
      outgoingCalls.map((c: any) => `| \`${c.name}\` | \`${c.filePath}\` | \`${c.uid}\` |`).join('\n');
  }

  // Affected processes
  const affectedProcessesContent = affectedProcesses.length === 0
    ? 'None'
    : affectedProcesses.map((p: any) => `\n- \`${p}\``).join('');
  ...
  if (affectedModules.length === 0) {
    ...
    affectedModulesContent = `| Module Name | Hits | Impact Type |\n| :--- | :--- | :--- |\n` +
      affectedModules.map((m: any) => `| \`${m.name}\` | ${m.hits} | \`${m.impact}\` |`).join('\n');
  }
  ...
  if (depthKeys.length === 0) {
    ...
    depthContent = `| Depth | Symbol Name | Relation | File Path | ID |\n| :--- | :--- | :--- | :--- | :--- |\n` +
      depthKeys.flatMap(depth =>
        byDepth[depth].map((item: any) => `| ${depth} | \`${item.name}\` | \`${item.relationType}\` | \`${item.filePath}\` | \`${item.id}\` |`)
      ).join('\n');
  }
