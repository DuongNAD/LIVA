...
fn test_panic_cleanup_check() {
    ...
    let result = std::panic::catch_unwind(move || {
        let _guard = TempDirGuard { path: temp_dir_clone };
        // Force assertion panic
        assert_eq!(1, 2, "Intentional panic for testing TempDirGuard drop cleanup");
    });
    
    assert!(result.is_err(), "Expected panic did not occur!");
    // The guard should have dropped, and the directory should be gone.
    assert!(!temp_dir.exists(), "Temporary directory was not cleaned up on panic unwind!");
}
