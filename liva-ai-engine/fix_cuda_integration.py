...
cuda_visual_studio_integration_dir = os.path.join(cuda_path, "extras", "visual_studio_integration", "MSBuildExtensions")

if not os.path.exists(cuda_visual_studio_integration_dir):
    print(f"❌ Could not find CUDA MSBuildExtensions at {cuda_visual_studio_integration_dir}")
    exit(1)
...
msbuild_extensions_dir = os.path.join(vs_path, "MSBuild", "Microsoft", "VC", "v170", "BuildCustomizations")
...
if not os.path.exists(msbuild_extensions_dir):
    ...
    if os.path.exists(vs_prog_path):
       msbuild_extensions_dir = os.path.join(vs_prog_path, "MSBuild", "Microsoft", "VC", "v170", "BuildCustomizations")
...
# Copy the files
files_to_copy = ["CUDA 12.1.props", "CUDA 12.1.targets", "CUDA 12.1.xml", "Nvda.Build.CudaTasks.v12.1.dll"]
copied = 0
