export const metadata = {
    ...
    parameters: {
        ...
        properties: {
            location: {
                type: "string",
                description: "Tên thành phố hoặc Vị trí địa lý. (Nếu để trống, AI sẽ tự động định vị người dùng qua Internet)."
            }
        },
        required: []
    }
...
export const execute = async (args: { location?: string }): Promise<string> => {
    try {
        ...
        if (!finalLocation) {
            console.log(`[Skill: get_weather_forecast] Đang xác định vị trí tự động qua IP...`);
            try {
                ...
                if (ipData && ipData.status === "success" && ipData.lat && ipData.lon) {
                    ...
                    console.log(`[Skill: get_weather_forecast] Đã tìm thấy vị trí: ${finalLocation} (${coords.lat}, ${coords.lon})`);
                }
            } catch (ipErr) {
                console.warn(`[Skill: get_weather_forecast] Lỗi định vị bằng IP:`, ipErr);
            }
        }

        if (finalLocation) {
            console.log(`[Skill: get_weather_forecast] Đang kiểm tra thời tiết cho khu vực: ${finalLocation}`);
        } else if (!coords) {
        ...
        if (!coords && finalLocation) {
            ...
            if (!coords) {
                ...
                console.log(`[Skill: get_weather_forecast] Tìm kiếm Geocoding: ${geoUrl}`);
        ...
        const conditionStr = weatherCodes[current.weather_code] || `(Mã thời tiết: ${current.weather_code})`;
