...
- **[v31 Core Platform Upgrade — Speculative Decoding + Parallel Routing + Streaming IPC Buffer]:**
  - **Speculative Decoding**: `ModelOrchestrator` spawns `llama-server` with draft model parameters (`-md` and `--draft 5`) when enabled via `LIVA_ENABLE_SPECULATIVE` and `LIVA_DRAFT_MODEL_NAME`, accelerating local inference token throughput.
  - **Parallel Path Routing**: `VirtualManager` executes parallelized I/O paths (e.g., executing multihop graph search and vector/KV recall concurrently) depending on the routed query path, and utilizes NFC-normalized Vietnamese word boundary matching (`isWholeWordMatch`).
  - **Streaming IPC Buffer**: `AgentLoop` optimizes token-streaming latency and rendering load by buffering tokens and flushing them only when meeting word/sentence punctuation or when the buffer length reaches 16 characters. Reduces IPC payload count by >80%.
