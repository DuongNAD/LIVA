...
export const execute = async (args: {
  ...
  try {
    ...
    const approved = await HITLGuard.requestApproval({
      ...
      args: { targetName: "Chính mình / Self (Zalo Bot)", message: finalMessage },
      reason: `Gửi thông báo qua Zalo Bot với nội dung: "${finalMessage.substring(0, 50)}..."`
    });
    if (!approved) {
      return "Lỗi: Người dùng từ chối gửi tin nhắn Zalo Bot này.";
    }
