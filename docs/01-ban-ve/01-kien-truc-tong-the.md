...
Đoạn code quyết định ở phía Tauri (`liva-desktop/src-tauri/src/lib.rs:355-368`) — bốn field
...
(`liva-desktop/src-tauri/src/lib.rs:460-464`).
...
| Lệnh `handle_command` có khác nhau không? | **Không** — cùng một hàm, cùng 42 nhánh lệnh |
...
Vỏ Tauri chỉ có **3 file `.rs`**: `main.rs` (7 dòng), `lib.rs` (577 dòng), `build.rs` (3 dòng).
...
> 📌 Nguồn đầy đủ (bảng opcode, chi tiết khung 9 byte, 42 lệnh `handle_command`): [Giao thức IPC và WebSocket](02-giao-thuc-ipc-va-websocket.md)

`start_websocket_server` đọc `LIVA_SERVER_PORT` (mặc định `8002`) và `LIVA_SERVER_HOST`
(mặc định `127.0.0.1`) rồi `TcpListener::bind` (`main.rs:446` trở đi). Vì vỏ Tauri không gọi
hàm này, **mọi mũi tên từ `liva-ui` vào cụm `GATEWAY` đều là nét đứt**.

### 2.4 Khối `CORE` — lõi Rust `liva-native-core` (`AppState`)

Đây là trái tim dự án; toàn bộ field trong `AppState` đều bọc `tokio::sync::Mutex`.

| Thành phần | Vai trò | Ghi chú then chốt |
|---|---|---|
| `handle_command` | Bộ định tuyến lệnh trung tâm — **42 nhánh** `match` + `_ => Err("Unknown command")` (`lib.rs:236-1484`) | Dùng chung cho **cả hai** profile. Sơ đồ ghi "41 lenh"; đếm thực tế trên code là 42 nhánh |
...
| `NativeMcpServer` | Server MCP nội bộ | **Không lệnh nào gọi tới** — xem §3.4 |
| `STDIO` | IPC dòng JSON qua stdin/stdout (`main.rs:358-433`) | Chỉ binary standalone |
...
Internet duy nhất** của hệ thống, và Obsidian vault (đích của `NativeMcpServer`, 0 caller).
...
3. **Có một mâu thuẫn hợp đồng ở chiều mic lên.** `useVoicePipeline.ts:345-350` gửi:
   ...
   ```
   tức header **1 byte**. Nhưng `VoiceFrame::decode` (`webrtc/frame.rs:29-53`) đòi header
   **9 byte** `[op u8][seq_id u32 LE][payload_size u32 LE]`. Server sẽ đọc 4 byte PCM đầu
   làm `seq_id`, 4 byte kế làm `payload_size` → gần như chắc chắn `>1 MiB` →
   `Err("Payload exceeds 1MB limit")` → `main.rs:573-576` log lỗi rồi `break`. Chiều
   **xuống** thì UI parse đúng 9 byte (`utils/speakerFrame.ts:5-13`, `App.vue:143-150`).
   `mobile_client/src/services/WebSocketClient.ts:226-235` (`serializeVoiceFrame`) tạo
   **đúng** 9 byte.

4. **`NativeMcpServer` được cấp phát nhưng không ai gọi.** Nó nằm trong `AppState`
   (`lib.rs:44`), khởi tạo ở cả `main.rs:168` và `src-tauri/src/lib.rs:347`, nhưng
   `handle_command` **không có nhánh `mcp:*` nào**; `list_tools()` (`mcp/server.rs:39`) có
   **0 caller kể cả test**.

5. **`set_eco_mode` là code chết ở phía UI.** Grep toàn repo (trừ `node_modules`): 2 hit,
   đều là định nghĩa (`lib.rs:82`) và đăng ký (`lib.rs:567`). `WidgetApp.vue:735` chỉ **xử lý
...
| IPC stdin/stdout | Dòng JSON, chỉ binary standalone (`main.rs:358-433`) | **[MỘT PHẦN]** |
...
| `WakeGate` (Rust) | Chỉ `liva-native-core.exe` (`main.rs:551`) | **[MỘT PHẦN]** |
...
| Lưu trữ | SQLite WAL, pool writer(1)+readers(4), `data/agents/liva_core/structured_memory.sqlite` | **[MỘT PHẦN]** — chỉ 3/15 bảng có writer |
| Mã hoá | AES-256-GCM, khoá từ `LIVA_ENCRYPTION_KEY` | **[MỘT PHẦN]** — chỉ 1 cột `facts.value` |
| `NativeMcpServer` | MCP nội bộ, đích là Obsidian vault; cấp phát ở cả hai điểm vào | **[THIẾU]** — 0 caller |
...
3. **Đường dữ liệu và đường lệnh đã tách, nhưng đường bộ nhớ chưa nối.** Sơ đồ cho thấy
   `HC → DB` là nét liền, nhưng chỉ 3/15 bảng thực sự có câu `INSERT` trong Rust
   (`facts`, `tasks`, `agent_checkpoints`) — phần còn lại của schema bộ nhớ dài hạn là
   **schema rỗng**.
...
- [Giao thức IPC và WebSocket](02-giao-thuc-ipc-va-websocket.md) — khung nhị phân 9 byte, bảng opcode, 42 lệnh `handle_command` (§2.3, §3.3)
...
- [Nợ kỹ thuật và rủi ro](../03-danh-gia/02-no-ky-thuat-va-rui-ro.md) — lấy 5 điểm ở §3 (lệch header 9 byte, MCP 0 caller, `set_eco_mode` chết)
...
- `liva-native-core/src/mcp/server.rs` — nếu có lệnh `mcp:*` được nối dây ⇒ sai điểm 4 của §3 và §4.1
