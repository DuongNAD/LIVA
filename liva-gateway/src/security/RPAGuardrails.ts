...
// [AI_CONTEXT §6 — Unbounded Map Cache] Bounded + auto-evicting rate-limit
// state. Keyed by skillName (practically <100 keys), but LRUCache guarantees
// stale windows are reclaimed instead of accumulating for the process lifetime.
// Mirrors SecurityGateway's LRUCache-based rate limiter for consistency.
const RATE_LIMIT_WINDOW_MS = 10_000; // 10 seconds
const RATE_LIMIT_MAX = 5; // max 5 actions per window per skill
const rateLimitState = new LRUCache<string, { count: number; windowStart: number }>({
    max: 500,
