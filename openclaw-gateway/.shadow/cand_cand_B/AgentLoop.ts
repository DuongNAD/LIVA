...
export class AgentLoop {
    ...
    public dispatch(task: MessageTask, token: AuthorityToken<AgentPhase>): void {
        if (!this.#authority.verify(token, this.#currentPhase)) throw new Error("Unauthorized Task Dispatch! Invalid Authority Token."); const laneQueue = this.#lanes.get(task.lane); if (laneQueue) { laneQueue.push(task); this.processLane(task.lane, token); }
    }
