...
unsafe fn get_active_window_info() -> (String, String) {
    ...
    let success = unsafe { QueryFullProcessImageNameW(process_handle, 0, path_buf.as_mut_ptr(), &mut size) };
...
#[cfg(windows)]
unsafe extern "system" fn keyboard_proc(code: i32, wparam: windows_sys::Win32::Foundation::WPARAM, lparam: windows_sys::Win32::Foundation::LPARAM) -> windows_sys::Win32::Foundation::LRESULT {
    use windows_sys::Win32::UI::WindowsAndMessaging::{HC_ACTION, WM_KEYDOWN, WM_SYSKEYDOWN, CallNextHookEx, KBDLLHOOKSTRUCT};
...
#[cfg(windows)]
unsafe extern "system" fn mouse_proc(code: i32, wparam: windows_sys::Win32::Foundation::WPARAM, lparam: windows_sys::Win32::Foundation::LPARAM) -> windows_sys::Win32::Foundation::LRESULT {
    use windows_sys::Win32::UI::WindowsAndMessaging::{
        HC_ACTION, WM_LBUTTONDOWN, WM_RBUTTONDOWN, WM_MBUTTONDOWN, MSLLHOOKSTRUCT, CallNextHookEx
    };
...
pub fn start_os_hook(tx: Sender<RawEvent>) -> Result<(), String> {
    ...
    use windows_sys::Win32::UI::WindowsAndMessaging::{
        SetWindowsHookExW, UnhookWindowsHookEx, GetMessageW, TranslateMessage, DispatchMessageW,
    ...
    std::thread::spawn(move || {
        unsafe {
            ...
            if k_hook == 0 || m_hook == 0 {
                if k_hook != 0 { UnhookWindowsHookEx(k_hook); }
                if m_hook != 0 { UnhookWindowsHookEx(m_hook); }
                return;
            }

            KEYBOARD_HOOK.store(k_hook, Ordering::SeqCst);
            MOUSE_HOOK.store(m_hook, Ordering::SeqCst);
            HOOK_THREAD_ID.store(windows_sys::Win32::System::Threading::GetCurrentThreadId(), Ordering::SeqCst);
...
pub fn stop_os_hook() -> Result<(), String> {
    use windows_sys::Win32::UI::WindowsAndMessaging::{PostThreadMessageW, UnhookWindowsHookEx, WM_QUIT};
