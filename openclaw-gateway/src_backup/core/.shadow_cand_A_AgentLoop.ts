...
export class AgentLoop {
    ...
    public dispatch(task: MessageTask, token: AuthorityToken<AgentPhase>): void {
        if (!this.#authority.verify(token, this.#currentPhase)) { throw new Error("Unauthorized Task Dispatch! Invalid Authority Token."); } const queue = this.#lanes.get(task.lane); if (queue) { queue.push(task); const worker = new Worker('taskWorker.js'); worker.postMessage({ task, token }); worker.onmessage = (event) => { if (event.data.error) { logger.error(`[AgentLoop] Lỗi tại [$${task.id}] (State: ${task.state}):`, event.data.error); } else { task.state = TaskState.COMPLETED; } }; }
    }
