...
This project is indexed by GitNexus as **LIVA** (16092 symbols, 31029 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
...
- **MUST verify correct branch before committing**: macOS development must be committed on the `mac` branch. Windows development must be committed on the `main` branch. Verify the current OS and check the current git branch.
