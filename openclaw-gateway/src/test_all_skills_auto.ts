...
async function runTests() {
    ...
    const dummyArgs: Record<string, any> = {
        ...
        "write_google_sheet": { spreadsheetId: "dummy_id", range: "A1:B2", values: [["Test", "Auto"]] },
        "search_google_drive": { query: "name contains 'LIVA'" }
    };

    console.log(`\n======================================================`);
    console.log(`BẮT ĐẦU KIỂM THỬ KHỐI LƯỢNG KỸ NĂNG CỦA LIVA (${skills.length} Skills)`);
    console.log(`======================================================\n`);
    ...
    for (const skill of skills) {
        ...
        try {
            ...
            const timeoutPromise = new Promise((_, reject) => 
                setTimeout(() => reject(new Error("Timeout (Quá 5 giây - Bỏ qua test này)")), 5000)
            );
