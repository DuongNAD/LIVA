...
IMPORTANT: All extracted text values (fact, entity, relation, sentiment, intent, topic_summary, category_routing_tag) MUST be written in English (en-US), even if the original conversation was in another language (e.g. Vietnamese). This saves substantial token space and standardizes cognitive semantic matching.

**Φ (Factual — Objective Data):**
- fact: Specific fact mentioned (who, what, when, where) in English
- entity: Proper nouns, place names, organizations in English (if any)
...
- category_routing_tag: Sub-category within the domain in English (e.g. "TypeScript", "Diet", "Git")
...
- relation: Social relationship mentioned in English
- sentiment: Overall emotional tone in English
- intent: User's underlying intent in English
- topic_summary: One-sentence summary of the conversation topic in English
...
export class ReflectionDaemon {
    ...
    #idleEpisodeTimer: NodeJS.Timeout | null = null;
    #isProcessing = false;
    ...
    public queueTurn(userMsg: string, aiReply: string): void {
        ...
        if (this.#segmenter) {
            ...
            // Schedule idle flush for episode (20s of silence triggers consolidation)
            this.#scheduleIdleEpisodeFlush();
        } else {
    ...
    #scheduleIdleEpisodeFlush(): void {
        if (this.#idleEpisodeTimer) {
            clearTimeout(this.#idleEpisodeTimer);
            this.#idleEpisodeTimer = null;
        }

        this.#idleEpisodeTimer = setTimeout(() => {
            if (this.#currentEpisode.length === 0) return;
            logger.info(`[ReflectionDaemon] 💤 Conversation went idle for 20s. Flushing current episode of ${this.#currentEpisode.length} turn(s) for extraction...`);
            ...
            this.#pendingQueue.push(...completedEpisode);
            this.processBatch().catch(e => {
                logger.warn(`[ReflectionDaemon] Idle background batch failed: ${e.message}`);
            });
        }, 20_000);
    }
