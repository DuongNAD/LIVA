...
- **Hybrid semantic recall:** `memory:search_hybrid` fuses vector similarity and FTS5 ranking; `memory:set_fact` / `memory:get_fact` manage structured facts (values encrypted with AES-256-GCM). Automatic recall is now wired into the chat path — each turn embeds the user's message and retrieves the top-`k` prior turns before the LLM call. **Caveat worth stating:** this needs a local embedding model at `models/embedding/` (384-dim, e.g. multilingual-e5-small). The weights are fetched out-of-band and are not in the repo; without them recall degrades to a no-op with a warning in the log rather than an error.
