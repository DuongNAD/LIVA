...
| `memory.cross-session-recall` | `agent/graph.rs`, `db.rs`, `llm/embedder.rs` | Chat/voice/Telegram scope → persist event+vector → hybrid vector/FTS retrieval → prompt | `e2e-memory.mjs`, DB/graph unit tests | A | Cần benchmark theo owner/conversation và regression corpus dài hạn |
...
| `agent.tool-runtime` | `agent/graph.rs::build_pipeline_graph`, `tool_calling.rs::execute_call`, `mcp/client.rs` | Intent/retrieval → resolve tool → policy cục bộ → native/MCP executor → observation | `tool_calling_probe.rs`, `mcp_client_e2e.rs`, `verify_commands.rs` | B | LLM path còn opt-in; thiếu corpus accuracy/latency và retrieval threshold trên chat thật |
...
| `distribution.windows-beta` | Tauri config, model manifest, installer/preflight scripts | Build UI → Tauri bundle → model/preflight validation | `check-installer-config.test.mjs`, release workflow | B | Thiếu signing và clean-machine matrix định kỳ |
| `security.desktop-boundaries` | `capabilities/default.json`, `tauri.conf.json`, `keystore.rs`, `crypto.rs` | WebView origin/CSP → IPC allow-list → native secret/data boundary | `crypto_boot_e2e.rs`, sandbox/command tests | B | Widget/dashboard/setup còn dùng chung capability; thiếu negative permission matrix |
