...
function finishEnrollmentSample() {
  ...
  const exactLength = Math.min(joined.length, Math.round(enrollmentSampleRate * ENROLLMENT_SECONDS))
  ...
  downloadedSamples += 1
  const sequence = String(downloadedSamples).padStart(2, '0')
  const link = document.createElement('a')
  const url = URL.createObjectURL(new Blob([wav], { type: 'audio/wav' }))
  link.href = url
  link.download = `hey_liva_positive_${sequence}.wav`
  link.click()
  ...
  enrollmentCount.textContent = `${downloadedSamples}/${ENROLLMENT_TARGET} mẫu đã tải`
  setStatus(`Đã tải mẫu Hey Liva ${sequence}`, 'success')
...
async function recordEnrollmentSample() {
  ...
  setStatus('Đang ghi 2,5 giây — nói “Hey Liva” ngay bây giờ', 'listening')
  log('Bắt đầu ghi một mẫu enrollment theo yêu cầu người dùng', 'info')
}
...
recordSampleButton.addEventListener('click', recordEnrollmentSample)
