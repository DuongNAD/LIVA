 ...
 */
export function validateVault(vaultRoot: string): VaultValidationReport {
  const report: VaultValidationReport = {
    totalFilesChecked: 0,
    invalidFilesCount: 0,
    brokenLinksCount: 0,
    errors: [],
  ...
  const allWikiLinks: { sourceFile: string; target: string; line: number }[] = [];
    ...
    if (fileErrors.length === 0) {
      ...
      if (!isTemplate) {
        ...
        if (!Array.isArray(frontmatter.tags)) {
          ...
          const hasLivaTag = frontmatter.tags.some(
            (tag: any) => typeof tag === 'string' && tag.startsWith('liva/')
          );
          if (!hasLivaTag) {
            fileErrors.push("At least one tag must start with 'liva/' (e.g. 'liva/skill')");
          }
          const nonStringTags = frontmatter.tags.filter((tag: any) => typeof tag !== 'string');
