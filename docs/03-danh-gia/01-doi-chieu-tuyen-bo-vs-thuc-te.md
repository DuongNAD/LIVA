...
| **Game-mode governor (`LIVA_GAME_MODE=auto`) hạ process priority** | README:26 | **[OK]** | `governor.rs:73-110` `game_mode_active()`, `governor.rs:124-172` `foreground_is_fullscreen()` (Win32 `GetForegroundWindow` + `GetWindowRect`, loại trừ Progman/WorkerW và chính mình), `governor.rs:179-192` `SetPriorityClass(BELOW_NORMAL)`. Nối dây ở **cả hai** entry: `main.rs:140-149`, `liva-desktop/src-tauri/src/lib.rs:452-456` | Bonus **không** có trong README: hạ `n_gpu_layers` khi game (`lib.rs:208-234` + `main.rs:268-293`), nhưng early-return nếu `LIVA_LLM_N_GPU_LAYERS=0` (mặc định) ⇒ mặc định không kích hoạt |
| **Deep Verification Suite (`verify_round2`, `router_stress`, `voice_stress`, `verify_duplex`)** | README:28 | **[OK]** | 4 file tồn tại trong `liva-native-core/src/bin/` (tổng 17 bin). `verify_duplex.rs:66` assert VAD <15 ms, `verify_duplex.rs:140` preemption <10 ms | Test suite thật nhưng nhỏ: `tests/` có ~21 `#[test]/#[tokio::test]` (integration_tests 5, sandbox_stress 6, self_correction_stress 6, swarm_stress 2, verify_commands 1, panic_cleanup 1) |
...
| **"Proactive / digest"** | `data/liva-config.json:38-60` | **[THIẾU]** | grep `proactive\|digest` trong Rust → chỉ `lib.rs:391` (`"proactiveEnabled": true` trong config mặc định) | 20+ khoá config **không có code đọc**. Người dùng bật/tắt trong `SettingsView.vue` chỉ ghi vào JSON, không có consumer |
...
| `webrtc/signaling.rs:24` — `TcpListener::bind("0.0.0.0:{port}")` | listener nội bộ, **bind `0.0.0.0`** | **(c)** | `SignalingServer` **không có caller nào** (grep toàn `src/` + `src-tauri/src/` chỉ ra định nghĩa ở `signaling.rs:13,17`) → code chết. Nếu ai nối dây lại, `0.0.0.0` phơi ra toàn LAN |
...
| `liva-native-core/Cargo.toml:26` — crate `webrtc = "0.12.0"` | — | **(c)** | Grep `use webrtc::` / `webrtc::api` / `webrtc::peer` trong `src/` = **0 hit**. Crate được khai báo nhưng **không dùng** → không có PeerConnection, **không có STUN/TURN** trong lõi |
...
- **`webrtc/signaling.rs:24` bind `0.0.0.0`** (dù hiện là dead code) — nếu ai nối dây lại sẽ phơi ra toàn LAN.
...
- **Về chủ đích thiết kế: yếu.** LIVA không có kill-switch mạng, không feature-gate `teloxide`/`reqwest`, không có kiểm tra runtime nào chặn kết nối ra ngoài. Nó offline **vì đường cloud chưa bao giờ được viết**, chứ không phải vì có ai chặn nó. Một PR duy nhất nối `AI_BASE_URL` vào LLM là tuyên bố đổ.
...
1. Đưa `mod telegram` + `teloxide`/`reqwest` vào `#[cfg(feature = "remote")]`, mặc định tắt → binary offline **không chứa** HTTP client nào.
...
3. Xoá `liva-native-core/static/` (chứa STUN Google), `webrtc/signaling.rs` (bind `0.0.0.0`, dead code), 2 call `safeFetch("http://127.0.0.1:3000/api/sensory-capture")`, và các key cloud chết trong `.env.example`.
...
- Governor game-mode Win32 thật, nối dây **cả hai** entry (`governor.rs:73-192`)
