...
export class PresenceDetector extends EventEmitter {
  ...
  private async getCurrentIdleTime(): Promise<number> {
    if (process.platform === "darwin") {
      try {
        const { stdout } = await execAsync("ioreg -c IOHIDSystem | awk '/HIDIdleTime/ {print $NF/1000000; exit}'", { timeout: 3000 });
        const parsed = parseFloat(stdout.trim());
        return isNaN(parsed) ? -1 : Math.round(parsed);
      } catch (error) {
        logger.warn(`[PresenceDetector] macOS ioreg command failed: ${error instanceof Error ? error.message : String(error)}`);
        return -1;
      }
