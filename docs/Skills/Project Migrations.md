...
# Skill: Project Migrations

## Description
Strict guidelines for creating, verifying, and running Sequelize PostgreSQL schema migrations in project-name.
...
Antigravity-Vibe uses TypeScript Sequelize migrations to track and evolve database schemas. These migrations do NOT run automatically on server boot, but are executed via the centralized automated runner `server/scripts/migrate.ts` or during production deploys.
...
## Verification Method
* Run `npx tsx server/scripts/migrate.ts --status` to verify pending and applied migrations.
