...
- 🗣️ **Wake word & game mode.** An optional "LIVA" wake word (`LIVA_WAKE_MODE`, four modes including a trained ONNX classifier per language) gates always-on listening. The resource governor (`LIVA_GAME_MODE=auto`) lowers process priority so LIVA never steals frames or cores from whatever you're doing. It fires on two independent signals: a fullscreen foreground app (Win32), **or** CPU load above `LIVA_BUSY_CPU_PERCENT` (default 80) — the latter catches renders and compiles that run in an ordinary window. The CPU figure subtracts LIVA's own usage, so generating a reply doesn't make LIVA throttle itself. GPU load is not read yet.
...
- The unified native core acting as the "Central Brain" and high-performance runner. It orchestrates the Agent loop (Planner/Executor), SQLite database administration (WAL + `sqlite-vec` + FTS5 hybrid search), and the Model Context Protocol (MCP) client/server ecosystem.
- Runs the entire realtime voice pipeline natively: Nemotron ASR + Piper TTS via ONNX runtimes (`ort`), Silero VAD, wake-word gating, and the WebRTC streaming duplex — plus local LLM routing/inference using `llama.cpp` bindings (`llama-cpp-2` crate).
...
- **Hybrid semantic recall:** `memory:search_hybrid` fuses vector similarity and FTS5 ranking; `memory:set_fact` / `memory:get_fact` manage structured facts (values encrypted with AES-256-GCM). Automatic recall is wired into **all three entry paths** — the voice pipeline, typed chat (`user_voice_command`), and the Telegram/API path (`chat:completion`) — each turn embeds the user's message and retrieves the top-`k` prior turns before the LLM call. Proven in a live session (`scripts/e2e-memory.mjs`): tell LIVA a fact, ask again — it answers correctly, **including after a full process restart** (the memory lives in SQLite, not RAM). **Caveat worth stating:** this needs a local embedding model at `models/embedding/` (384-dim, e.g. multilingual-e5-small). The weights are fetched out-of-band and are not in the repo; without them recall degrades to a no-op with a warning in the log rather than an error.
- **Obsidian Knowledge Vault over MCP:** The MCP server embedded in the Rust core is reachable from the command dispatcher via `mcp:list_tools` / `mcp:call_tool`; every file operation is pinned under `LIVA_VAULT_PATH` and rejects absolute paths and `..`. A separate, more mature **TypeScript** server also lives in `teamwork_projects/obsidian_llm_wiki/`.
